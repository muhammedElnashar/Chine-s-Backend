                    ...
                    <form method="POST" action="{{ route('users.update',$user) }}" enctype="multipart/form-data">
                        ...
                        <div class="mb-7">
                            ...
                            <input type="text" name="name" value="{{ old('name',$user->name) }}"
                                   class="form-control form-control-solid mb-2" placeholder="Enter Username"/>
                            @error('name')<small class="text-danger">{{ $message }}</small>@enderror
                        </div>
                        <div class="mb-7">
                            <label class="form-label required">Email</label>
                            <input type="email" name="email" value="{{ old('email',$user->email) }}"
                                   class="form-control form-control-solid mb-2" placeholder="Enter Email"/>
                            ...
                            <select name="role" aria-label="Select Role" data-control="select2"
                                ...
                                @foreach(\App\Enum\UserRoleEnum::cases() as $type)
                                    @if(in_array($type->value, ['admin', 'super_admin']))
                                        <option
                                            value="{{ $type->value }}" {{ old('type',$user->role) == $type->value ? 'selected' : '' }}>
                        ...
                        <div>
                            @if($user->image)
                                <img src="{{\Illuminate\Support\Facades\Storage::disk('s3')->url($user->image)}}"
                                     alt="User Image" class="img-thumbnail" style="max-width: 50px; max-height: 200px;">
                            @endif
                        </div>
